...
export default function RestaurantDetail() {
  ...
  useEffect(() => {
    ...
    Promise.all([
      ...
      orderId
        ? api.reviews.getSuggestions(orderId).catch(() => null)
  ...
  const updateCart = (item, delta) => {
    ...
    if (
      ...
      updatedCart[0].restaurant_id !== restaurantId
    ) {
  ...
  return (
    <div className="min-h-screen bg-gray-50 pb-28">
      {/* Hero */}
      <div className="relative h-64 md:h-80 bg-gradient-to-br from-emerald-100 via-green-100 to-lime-100 overflow-hidden">
        {restaurant.image ? (
          <img
          ...
          <div className="flex items-end justify-between">
            ...
            <div className="flex gap-2">
              <button
                ...
                className={`p-2.5 rounded-full backdrop-blur-sm transition-all ${isWishlisted ? "bg-emerald-500 text-white shadow-lg shadow-emerald-200" : "bg-white/20 text-white hover:bg-white/30"}`}
              >
                <Heart
        ...
        <div className="max-w-5xl mx-auto px-4 sm:px-6 py-4">
          <div className="flex flex-wrap items-center gap-x-6 gap-y-2 text-sm">
            ...
            <div className="flex items-center gap-1.5 text-gray-500">
              <span
                className={`w-2 h-2 rounded-full ${restaurant.is_open ? "bg-emerald-500" : "bg-gray-400"}`}
              />
          ...
          <div className="flex gap-1">
            {["menu", "reviews", "info"].map((tab) => (
              <button
                ...
                className={`px-5 py-3.5 text-sm font-bold capitalize transition-colors border-b-2 ${
                  activeTab === tab
          ...
          <div className="space-y-6">
            ...
            <div className="bg-white rounded-2xl p-6 shadow-sm border border-gray-100">
                    ...
                    {suggestions.suggestions.map((s, i) => (
                      <button
                        ...
                        onClick={() =>
                          setNewReview((r) => ({
                            ...r,
                            comment: r.comment + (r.comment ? " " : "") + s,
                          }))
                        }
                        className="text-xs bg-emerald-50 hover:bg-orange-100 text-orange-700 font-medium px-2.5 py-1.5 rounded-lg border border-emerald-200 transition-colors"
                      >
                        +{s}
                      </button>
                    ))}
              ...
              <textarea
                ...
                className="w-full border border-gray-200 rounded-xl p-3 text-sm resize-none h-24 outline-none focus:border-emerald-500 focus:ring-2 focus:ring-emerald-100"
              />
              <Button
                onClick={submitReview}
                className="mt-3 bg-emerald-500 hover:bg-emerald-600 text-white rounded-xl"
              >
                Submit Review
              </Button>
            </div>
          ...
          <div className="bg-white rounded-2xl p-6 shadow-sm border border-gray-100 space-y-4">
            ...
            <div className="grid grid-cols-2 gap-4 pt-2 text-sm">
              <div className="flex items-center gap-2 text-gray-600">
                <Clock className="w-4 h-4 text-emerald-500" />
                {restaurant.opening_hours || "9am - 10pm"}
              </div>
              <div className="flex items-center gap-2 text-gray-600">
                <Phone className="w-4 h-4 text-emerald-500" />
                {restaurant.phone || "Not listed"}
              </div>
              <div className="flex items-center gap-2 text-gray-600">
                <MapPin className="w-4 h-4 text-emerald-500" />
      ...
      {cartCount > 0 && (
        <div className="fixed bottom-6 left-1/2 -translate-x-1/2 z-50 w-full max-w-sm px-4">
          <button
            ...
            className="w-full bg-gradient-to-r from-emerald-500 via-green-500 to-lime-400 text-white py-4 px-6 rounded-2xl shadow-2xl shadow-emerald-300 flex items-center justify-between hover:from-emerald-600 hover:to-lime-500 transition-all"
          >
            <div className="flex items-center gap-3">
              <span className="bg-white/25 rounded-lg px-2 py-0.5 font-black text-sm">
                {cartCount}
              </span>
