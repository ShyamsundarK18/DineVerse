  ...
  try {
    if (req.user.role !== 'customer') {
      return res.status(403).json({
        success: false,
        message: 'Forbidden. Only accounts with the customer role can place orders.'
      });
    }
...
const payOrder = async (req, res) => {
  try {
    ...
    const paymentSuccess = Math.random() < 0.99;
    if (paymentSuccess) {
...
// 🛠️ UPDATED FOR LIVE COORDINATES PUSHES
const updateOrderStatus = async (req, res) => {
  try {
    const { status, courier_lat, courier_lng } = req.body;

    const updateData = { status };
    if (courier_lat !== undefined) updateData.courier_lat = courier_lat;
    if (courier_lng !== undefined) updateData.courier_lng = courier_lng;
...
// 🛠️ UPDATED FOR PATCHING RAW COORDINATE BODY EMISSIONS
