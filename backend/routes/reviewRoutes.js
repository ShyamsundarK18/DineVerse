...
// 1. DYNAMIC REVIEWS FOR RESTAURANT
...
// AI keyword suggestions (FIXED FOR NEW path-to-regexp SYNTAX)
// We provide two rigid paths to safely support both param and query-string formats
router.get(['/suggestions', '/suggestions/:orderId'], async (req, res) => {
  try {
    // Safely reads from path parameters or query parameters
    ...
    const itemNames = (order.items || []).map(item => item.name ? item.name.toLowerCase() : '');
    const suggestions = [];
...
// 3. BASE GET ROUTE (Keep below specific endpoints like /suggestions)
