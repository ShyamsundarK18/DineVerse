async function testAuth() {
  ...
  const resReq = await fetch('http://localhost:8080/api/auth/register', {
    method: 'POST',
  ...
  console.log('\nTesting Login...');
  const loginReq = await fetch('http://localhost:8080/api/auth/login', {
    method: 'POST',
    headers: { 'Content-Type': 'application/json' },
  ...
  console.log('\nTesting Get Profile...');
  const profileReq = await fetch('http://localhost:8080/api/auth/profile', {
    method: 'GET',
